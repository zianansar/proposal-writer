...
describe('ProposalAnalyticsDashboard', () => {
  ...
  it('renders empty state when totalProposals is 0', async () => {
    ...
    expect(
      await screen.findByText('No analytics data yet. Generate and track proposals to see insights here.')
    ).toBeTruthy();
  });
  ...
  it('response rate color is green when > 20%', async () => {
    mockInvoke.mockImplementation((cmd: string) => {
      if (cmd === 'get_proposal_analytics_summary') return Promise.resolve({ ...mockSummary, responseRate: 25.0 });
      return Promise.resolve([]);
    });
  ...
  it('response rate color is yellow when 10-20%', async () => {
    mockInvoke.mockImplementation((cmd: string) => {
      if (cmd === 'get_proposal_analytics_summary') return Promise.resolve({ ...mockSummary, responseRate: 15.0 });
      return Promise.resolve([]);
    });
  ...
  it('response rate color is red when < 10%', async () => {
    mockInvoke.mockImplementation((cmd: string) => {
      if (cmd === 'get_proposal_analytics_summary') return Promise.resolve({ ...mockSummary, responseRate: 5.0 });
      return Promise.resolve([]);
    });
  ...
  it('shows N/A when no best strategy', async () => {
    mockInvoke.mockImplementation((cmd: string) => {
      if (cmd === 'get_proposal_analytics_summary') return Promise.resolve({ ...mockSummary, bestStrategy: null, bestStrategyRate: 0 });
      return Promise.resolve([]);
    });
