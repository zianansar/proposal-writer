    ...
    return (
      <div data-testid="virtualized-list">
        {Array.from({ length: itemsToRender }).map((_, index) => {
          ...
          return <div key={key}>{children({ index, style: { top: index * 72, height: 72 } })}</div>;
        })}
      </div>
    );
  },
...
describe("ProposalHistoryList", () => {
  ...
  const mockDefaultInvoke = (
    proposalResponse: ProposalHistoryResponse = singleProposalResponse,
  ) => {
  ...
  it("does not render DatabaseExportButton in empty state (7.6)", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /export encrypted database backup/i }),
