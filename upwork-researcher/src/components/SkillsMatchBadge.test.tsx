...
describe("SkillsMatchBadge", () => {
  ...
  it("has correct aria-label with percentage and fit level (NFR-14)", () => {
    ...
    const badge = screen.getByTestId("skills-match-badge");
    expect(badge).toHaveAttribute("aria-label", "Skills match percentage: 67%, moderate fit");
  });
  ...
  it('has "strong fit" aria-label for >= 75%', () => {
    ...
    const badge = screen.getByTestId("skills-match-badge");
    expect(badge).toHaveAttribute("aria-label", "Skills match percentage: 80%, strong fit");
  });
  ...
  it('has "weak fit" aria-label for < 50%', () => {
    ...
    const badge = screen.getByTestId("skills-match-badge");
    expect(badge).toHaveAttribute("aria-label", "Skills match percentage: 30%, weak fit");
  });
  ...
  it("shows tooltip with matched skill counts when provided", () => {
    render(<SkillsMatchBadge percentage={67} matchedCount={2} totalCount={3} />);
