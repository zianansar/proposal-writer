...
describe('JobScoreBadge', () => {
  ...
  it('applies correct CSS class per color flag', () => {
    const { rerender } = render(
      <JobScoreBadge overallScore={50} colorFlag="green" />
    );
  ...
  describe('Clickable badge with onToggle', () => {
    ...
    it('calls onToggle when Escape key is pressed and isExpanded is true (AC #5)', async () => {
      const user = userEvent.setup();
      const onToggle = vi.fn();
      render(<JobScoreBadge overallScore={75.0} colorFlag="yellow" onToggle={onToggle} isExpanded={true} />);
    ...
    it('does not call onToggle when Escape key is pressed and isExpanded is false', async () => {
      const user = userEvent.setup();
      const onToggle = vi.fn();
      render(<JobScoreBadge overallScore={75.0} colorFlag="yellow" onToggle={onToggle} isExpanded={false} />);
    ...
    it('updates aria-expanded based on isExpanded prop', () => {
      const onToggle = vi.fn();
      const { rerender } = render(
        <JobScoreBadge overallScore={75.0} colorFlag="yellow" onToggle={onToggle} isExpanded={false} />
      );

      expect(screen.getByRole('button')).toHaveAttribute('aria-expanded', 'false');

      rerender(<JobScoreBadge overallScore={75.0} colorFlag="yellow" onToggle={onToggle} isExpanded={true} />);
